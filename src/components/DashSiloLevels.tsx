...
function RadarIcon() {
  return (
    <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
      <path d="M12 12h.01" /><path d="M8.5 8.5a5 5 0 0 0 0 7" /><path d="M15.5 8.5a5 5 0 0 1 0 7" />
      <path d="M5.7 5.7a9 9 0 0 0 0 12.6" /><path d="M18.3 5.7a9 9 0 0 1 0 12.6" />
    </svg>
  );
}
...
function SiloGauge({ silo, index, narrow }: { silo: SiloLevel; index: number; narrow: boolean }) {
  ...
  // Full usable capacity is drawn at 80% of the vessel so the real radar head
  // space remains visible even when the operational reading is 100% full.
  ...
  return (
    <article style={{ ...cardStyle, display: "flex", flexDirection: "column", minHeight: narrow ? 390 : 330 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: narrow ? "1fr" : "minmax(0, 1fr) 105px", alignItems: "center", gap: 8, flex: 1, marginTop: 2 }}>
        <svg viewBox="0 0 300 270" role="img" aria-label={`Feed Silo ${index + 1} is ${Math.round(pct)}% full`} style={{ width: "100%", height: narrow ? 225 : 240, overflow: "visible" }}>
          <defs>
            <clipPath id={clipId}><path d="M58 42 Q140 15 222 42 L222 166 L166 236 H114 L58 166 Z" /></clipPath>
          ...
          <circle cx="140" cy="33" r="4" fill="#08717A" /><path d="M133 40q7-7 14 0" fill="none" stroke="#08717A" strokeWidth="1.5" />
        ...
        <div style={{ minWidth: 0, display: narrow ? "grid" : "block", gridTemplateColumns: narrow ? "1.25fr 1fr 0.8fr" : undefined, gap: narrow ? 10 : undefined, alignItems: "end" }}>
          <div>
            ...
            <div style={{ fontFamily: "var(--font-d)", fontSize: narrow ? 30 : 38, fontWeight: 800, letterSpacing: "-0.04em", color: pct < 15 ? RED : INK, lineHeight: 1.05, marginTop: 4 }}>{Math.round(pct)}%</div>
          ...
          <div style={{ fontSize: 9.5, lineHeight: 1.5, color: "var(--t3)", marginTop: narrow ? 0 : 13 }}>
            <div>Head space</div><strong style={{ color: "var(--t2)", fontSize: 11 }}>{silo.distanceMm.toLocaleString("en-ZA")} mm</strong>
          </div>
          {silo.battery != null && <div style={{ fontSize: 9.5, color: "var(--t3)", marginTop: narrow ? 0 : 8 }}>Battery<br /><strong style={{ color: "var(--t2)" }}>{Math.round(silo.battery)}%</strong></div>}
        </div>
      </div>
  ...
  return (
    <section aria-label="Silo feed levels">
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10, marginBottom: 10, flexWrap: "wrap" }}>
        <div><span style={{ fontFamily: "var(--font-d)", fontWeight: 800, fontSize: 15, color: "var(--primary)" }}>Silo feed levels</span><span style={{ fontSize: 10, color: "var(--t3)", marginLeft: 8 }}>Radar level monitoring</span></div>
      ...
      {silos.length > 0 ? (
        ...
        <div style={{ ...cardStyle, minHeight: 128, display: "grid", placeItems: "center", textAlign: "center" }}>
          <div><div style={{ color: "var(--t2)", fontSize: 12, fontWeight: 700 }}>No silo radar readings available</div><div style={{ color: "var(--t3)", fontSize: 10.5, marginTop: 5 }}>Waiting for a physical silo sensor to report.</div></div>
        </div>
      )}
      {silos.length > 0 && data && <div style={{ fontSize: 9.5, color: "var(--t3)", marginTop: 7, textAlign: "right" }}>100% usable fill is calibrated at {data.calibration.fullDistanceMm.toLocaleString("en-ZA")} mm head space.</div>}
    </section>
  );
}
