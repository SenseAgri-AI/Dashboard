// Temporary cutover fallback. Remove after every enabled farm has a notifier heartbeat.
...
import { powerOutageAlert, logsOverdueAlert, nightDisturbanceAlert, sleepDeclineAlert, heatStressAlert, type Alert, type NoisePoint } from "@/lib/legacyAlerts";
...
export async function readLegacyAlerts(farm: FarmConfig) {
  ...
  return { alerts, updatedAt: new Date(now).toISOString(), source: "legacy" as const };
}
